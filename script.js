// ===== SCENARIOS =====
// (Use the full 50+ scenarios array we created previously)
let scenarios = [
...
];// replace with the full array above

// ===== DAILY MISSIONS =====
// (Use the 50+ daily missions array we created)
let dailyMissions = [
...
function loadScenario(){
  ...
  scenario.options.forEach((opt,index)=>{
    ...
    btn.onclick = ()=>{
      ...
      updateDashboard();
      if(opt.score>0 && streak%5===0) triggerConfetti(); // confetti on level up
    };
...
function triggerConfetti(){
  ...
  const confettiLoop = setInterval(()=>{
    count++;
    if(count>200){ clearInterval(confettiLoop); confettiActive=false;}
  },16);
}

drawConfetti();let scenarios = [
  {text:"She replies 'lol' to your message.", options:[
    {reply:"Keep it cool.", feedback:"Neutral, maintains frame.", score:1},
    {reply:"Why are you laughing?", feedback:"Weak, insecure.", score:-1},
    {reply:"Haha, good one.", feedback:"Safe, boring.", score:0}
